...
FOOTBALL_CONTEXT_TERMS = {
    ...
    "training", "traint", "opstelling", "doelpunt", "scoort", "speelt", "fit", "afwezig", "spits", "keeper", "verdediger",
...
def person_patterns():
    ...
    for row in rows:
        ...
        if not name:
            continue
        result.append((name.lower(), name, "volledige naam"))
        parts = name.split()
        if len(parts) >= 2:
            surname = (
                " ".join(parts[-2:])
            ...
            if len(surname) >= 4:
                result.append((surname.lower(), name, "achternaam"))
    return result
...
def relevance_details(title, source, config):
    ...
    if any(
        contains_phrase(lowered, term.lower())
    ...
    for term in config.get("club_terms", []):
        if contains_phrase(lowered, term.lower()):
            return True, f"FC Twente-term: {term}"

    has_football_context = any(
        contains_phrase(lowered, term)
        for term in FOOTBALL_CONTEXT_TERMS
    )

    for pattern, display_name, match_type in person_patterns():
        if contains_phrase(lowered, pattern):
            if match_type == "volledige naam" or has_football_context or len(pattern) >= 7:
                return True, f"Persoon: {display_name}"
...
def fetch_google_news(source, config):
    queries = ['"FC Twente" OR "F.C. Twente"']
    full_names = [
        f'"{display_name}"'
        for _, display_name, match_type in person_patterns()
        if match_type == "volledige naam"
    ]
    for index in range(0, len(full_names), 8):
        queries.append(" OR ".join(full_names[index:index + 8]))

    found = {}
    for query in queries:
        rss_url = (
            "https://news.google.com/rss/search?q="
            + quote_plus(query + " when:2d")
        ...
        for item in root.findall(".//item"):
            raw_title = normalize_title(item.findtext("title"))
            link = normalize_title(item.findtext("link"))
            if not raw_title or not link:
                continue

            title = raw_title
            publisher = "Onbekende bron"
            if " - " in raw_title:
                title, publisher = raw_title.rsplit(" - ", 1)

            is_relevant, match_reason = relevance_details(
                title,
                {"source_scope": "strict"},
                config,
            )
            ...
            found[link] = {
                "source": f"Google Nieuws · {publisher}",
                "title": title,
                "url": link,
                "found_at": now_iso(),
                ...
                "fingerprint": fingerprint(title),
                "match_reason": match_reason,
                "reliability": int(source.get("reliability", 4)),
                "source_type": "Nieuwszoeker",
            }
...
def fetch_source(source, config):
    if source.get("type") == "google_news":
        return fetch_google_news(source, config)
    return fetch_site(source, config)
    ...
    try:
        ...
        for source in config.get("sources", []):
            if not source.get("enabled", True):
                continue
            try:
                articles = fetch_source(source, config)
                baseline = (
...
def cleanup_irrelevant_articles():
    ...
    try:
        ...
        for row in rows:
            ...
            if source_name.startswith(("X ·", "Google Nieuws ·")):
